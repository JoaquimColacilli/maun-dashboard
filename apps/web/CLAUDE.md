...
- **El padding inferior del `<main>` reserva todo lo que flota abajo, no solo la barra.** Mientras `IndicadorSync` está a la vista («Sin conexión», «Sincronizando…»), `Marco` mide cuánto ocupa desde el borde de la pantalla y el padding pasa a ser eso más un margen, en todos los anchos. Sin eso, la píldora quedaba justo sobre el último control de la pantalla, que en el taller sin señal es casi siempre. Un elemento fijo nuevo que flote sobre el contenido tiene que entrar en esa cuenta. `lo-que-flota-abajo.spec.ts` recorre las pantallas hasta el final del scroll, con señal y sin señal.
