...
- En el celular, lista con `TiraDelMes` (`role="group"` con `aria-pressed`). En tablet y PC, `GrillaDelMes` (dos eventos por celda en tablet, tres en la PC, y «+N más») y **el panel del día, que está siempre**: sin día elegido muestra hoy, y volver a tocar el día elegido, «Hoy» o Escape vuelven a hoy.
- **La grilla no cambia de ancho al elegir un día.** El panel es una columna fija de una consulta de contenedor (`@container/agenda`: 340 px desde 58rem, 380 desde 68rem), y la grilla se dimensiona sin contarlo. No le vuelvas a poner botón de cerrar en ese ancho ni lo montes solo con un día elegido: eso era lo que encogía el calendario. Debajo de 58rem el panel es una capa encima de la grilla (`absolute` en la misma celda de la grilla CSS, que queda como bloque contenedor), y ahí sí se cierra. `agenda.spec.ts` mide el ancho de la grilla en los dos casos.
- **La hoja del día es un `<dialog>` modal y deja inertes los avisos globales**: el deshacer adentro va por el aviso local de `DetalleDelDia` (`aviso`). Si movés una acción con deshacer a otra hoja, pasa lo mismo.
...
- **En la PC, el panel del día muestra hoy aunque nadie lo haya elegido**: un texto de hoy aparece dos veces en el `<main>`, en la celda y en el panel. Buscalo dentro de la celda (`laAnotacionDeHoy`) o del panel (`getByRole('complementary')`).
