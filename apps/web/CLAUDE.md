...
- **Un monto que vive en una tarjeta es `MontoQueEntra`** (`@/shared/ui`, ADR 0060): la letra sale del ancho del contenedor más cercano y del largo del monto, entre 13 px y su tope, en un renglón. El contenedor necesita `@container` y sus hijos de flex o de grid, `min-w-0`. En una fila de tarjetas se le pasa `caracteres` con el más largo (`caracteresDe`), para que todas lleven la misma letra. **Nunca se redondea, se trunca, se tapa con `overflow` ni se cortan los centavos**; si algo no entra, `montos-en-las-tarjetas.spec.ts` lo mide y falla. Ese spec también mide todo el texto de las tarjetas de plata: si agregás una, sumala a `TARJETAS_DE_PLATA`.
...
- **`CaminoDeHitos` no pregunta por etapas**: recibe `hitos` y la `nota` de `notaDelRelevamiento` y dibuja. El estimativo, cuando lo hubo, es un paso más adelante (seis columnas en horizontal en vez de cinco). **El relevamiento es una (i) con una nota, no un paso ni un casillero** (ADR 0059): una sola, a la derecha de la fecha del paso en curso (sola en esa línea si el paso no tiene fecha), y su resumen va en la línea de abajo del titular. En el celular abre una hoja (`<dialog>` modal, anotada con `useAlgoEnCurso`); desde 768 px, un popover anclado al `<li>` que abre también al pasar el mouse. Qué texto, qué día y cuándo aparece se decide en el dominio.
...
- **La réplica tiene `staleTime` de 60 segundos: algo que se escribe en la base desde afuera de la app (por REST, o el cliente contestando la encuesta) no aparece con un `goto`.** La app arranca con lo guardado y no vuelve a pedir. Para traerlo, «Sincronizar ahora» en Ajustes, que es el camino sin gesto (`sincronizarEIrAInicio` en `opiniones.spec.ts`), o crear los datos antes de abrir la app.
