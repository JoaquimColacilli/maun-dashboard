...
- **El bloqueo cuenta el tiempo afuera, no las aperturas** (ADR 0026). La marca guarda `desbloqueadaEn` y `salioEn`; al abrir y al volver de segundo plano se pide la huella si pasó un minuto (`UMBRAL_DEL_BLOQUEO_MS`) desde la última vez adentro. Una recarga no la pide. **No lo cambies por `sessionStorage`**: sobrevive a la restauración de la app y el bloqueo deja de existir.
- **`salioEn` se escribe al ocultarse estando a la vista, nunca en un `pagehide` con el documento ya oculto**: cerrar desde recientes una app que estaba hace una hora afuera renovaría el momento. El tipo de navegación (`reload`) solo evita el pedido si la app nunca salió a segundo plano desde el desbloqueo, con un tope de diez minutos: ensancharlo estira el bloqueo en las plataformas que informan una restauración como recarga.
...
- **Tirar hacia abajo no recarga**: `overscroll-behavior-y: none` en `html` y `body`. El service worker está en `prompt` y solo recarga después de tocar «Actualizar».
