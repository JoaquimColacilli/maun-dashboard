...
- **La grilla del mes ocupa todo el ancho del área de contenido, siempre, y el panel del día va encima** (ADR 0034). Es una capa `absolute` sobre el área de la grilla (el contenedor `relative` que la envuelve): arranca cerrada, tiene «Cerrar el día» en todos los anchos y se cierra con Escape. Al abrirse, el foco va al panel; al cerrarse, vuelve al botón del día. No la pongas al costado ni la dejes abierta mostrando hoy: le roba ancho a la grilla. `agenda.spec.ts` mide la grilla contra el área de contenido, con el panel abierto y cerrado, a 1440 y a 1024 px.
...
- **En la PC, con el panel del día abierto, sus textos también están en la celda**: un texto aparece dos veces en el `<main>`. Buscalo dentro de la celda (`laAnotacionDeHoy`, `celda`) o del panel (`getByRole('complementary')`).
