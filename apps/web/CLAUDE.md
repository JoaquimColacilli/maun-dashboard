...
- **`CaminoDeHitos` no pregunta por etapas**: recibe `hitos` y `relevamiento` de `vistaDelCliente` y dibuja. El estimativo, cuando lo hubo, es un paso más adelante (seis columnas en horizontal en vez de cinco), y el casillero del relevamiento va adentro del paso del presupuesto, en blanco o tildado con el mismo verde de los pasos hechos. Qué texto, qué día y cuándo aparece se decide en el dominio (ADR 0058).
