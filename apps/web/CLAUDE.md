...
- En el celular, lista con `TiraDelMes`; en tablet y PC, `GrillaDelMes` (dos eventos por celda en tablet, tres en la PC, y «+N más»). La tira es `role="group"` con `aria-pressed`.
...
- **En el celular la agenda no está en la barra** (decisión con el dueño, objeción en el ADR): se llega por «Hoy en la agenda» de Inicio, y «Anotar algo» es la primera acción del botón redondo.
