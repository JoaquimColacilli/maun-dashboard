    ...
    <fieldset className="flex flex-col gap-1.5">
      ...
      <div className="grid grid-cols-3 gap-0.5 rounded-field bg-surface-2 p-1">
        {OPCIONES.map((opcion) => (
          <label
            key={opcion.id}
            className="flex min-h-tap cursor-pointer items-center justify-center gap-1.5 rounded-control px-1 text-center text-label leading-tight font-medium text-text-2 has-checked:bg-elevado has-checked:font-semibold has-checked:text-ink has-checked:shadow-float has-focus-visible:outline-2 has-focus-visible:outline-offset-2 has-focus-visible:outline-ink"
          >
            <input
              type="radio"
              name={nombre}
