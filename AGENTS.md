...
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| NUMA                  | El nombre de la app: la pestaña, el manifiesto, el logo, el acceso y los avisos del sistema. Por «NUevas MAneras» de gestionar el taller (ADR 0073). El logo y el ícono salen de `@maun/ui/marca`.                                                                                        | Ninguna: la base no nombra a la app.                                                                      |
| MAUN                  | El taller (MAUN Muebles) y su tesoro, y lo que ve un cliente en `/v/` y `/o/`: la M, «Taller MAUN». También el nombre interno: `@maun/*`, las claves `maun.*` y `maun:*`, la base de IndexedDB `maun`, los `@keyframes maun-*` y el dominio. Eso no se renombra: carga datos o identidad. | El tesoro `maun`, los ajustes `maun.*`, el scope `@maun/*` en los `comment on`.                           |
