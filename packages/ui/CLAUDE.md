...
- **Lo que ve el cliente no dibuja un mueble** (ADR 0069): nunca va a ser el suyo. `TrabajoEnEtapa` dibuja lo que está sobre la mesa en cada etapa (el anotador, el presupuesto, la seña encima, el serrucho, la casa) y Gracias es una tarjeta firmada. El mueble queda para las escenas de la app del dueño.
- **Los planos y las cajas ya existen**: `planoDelPiso`, `planoDeFrente`, `planoDeCostado` y `planoInclinado` para dibujar en una cara, y `limites`, `encerrar`, `unir` y `correr` para la caja de lo dibujado, en `proyeccion.ts`; `Cara` en `trazos.tsx` es un polígono con su relleno desde puntos en 3D. La caja tiene que ser la de lo que se ve, marcas de mano incluidas: con el ancho fijo, el lienzo la centra en los dos ejes.
