...
export interface CarcasaProps extends Ubicacion {
  largo: number;
  profundidad: number;
  alto: number;
}

export function Carcasa({ x, y, z, largo, profundidad, alto }: CarcasaProps) {
  const tabla = 3;
  const zocalo = 7;
  const base = z + zocalo;
  const cuerpo = alto - zocalo - 3;
  const interior = largo - 2 * tabla;
  const canto = { izquierda: 'tinta', derecha: 'costado' } as const;
  return (
    <g>
      <Caja
        x={x + 3}
