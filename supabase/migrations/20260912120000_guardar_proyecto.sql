-- Guardar un proyecto es una sola operación. El proyecto, sus pagos y sus gastos son 1 + N + M
-- filas en tres tablas: encolarlas por separado rompe por el orden de la foreign key, deja medio
-- proyecto si una falla, y como la cola drena de a una, un rechazo definitivo de cualquiera de
-- ellas tapa a las demás. supabase-js no agrupa varias queries en una transacción: el camino
-- soportado para lógica multi-sentencia es una función llamada por rpc(), con los arrays en jsonb
-- desarmados del lado de Postgres (ADR 0015).


create function public.guardar_proyecto(p_proyecto jsonb, p_pagos jsonb, p_gastos jsonb)
returns jsonb
language plpgsql
security invoker
set search_path = ''
...
begin
  ...
  if exists (
    select 1
    from jsonb_to_recordset(p_pagos) as r (id uuid, fecha date, monto_centavos bigint, borrado boolean)
    where r.id is null
       or (not coalesce(r.borrado, false) and (r.fecha is null or r.monto_centavos is null))
  ) then
  ...
  if exists (
    select 1
    from jsonb_to_recordset(p_gastos) as r (id uuid, fecha date, monto_centavos bigint, borrado boolean)
    where r.id is null
       or (not coalesce(r.borrado, false) and (r.fecha is null or r.monto_centavos is null))
  ) then
  ...
  if v_existia then
    ...
    -- entonces no hace nada, porque el upsert de abajo y las bajas ya son no-op.
  ...
  -- Alta e edición se escriben por separado, no con un upsert. En un `insert ... on conflict do
  ...
  select r.id, v_fila.id, r.fecha, coalesce(r.concepto, ''), r.monto_centavos
  from jsonb_to_recordset(p_pagos) as r (
    id uuid, fecha date, concepto text, monto_centavos bigint, borrado boolean
  )
  ...
  select r.id, v_fila.id, r.fecha, coalesce(r.descripcion, ''), r.monto_centavos
  from jsonb_to_recordset(p_gastos) as r (
    id uuid, fecha date, descripcion text, monto_centavos bigint, borrado boolean
  )
  where not coalesce(r.borrado, false)
  on conflict (id) do update set
    proyecto_id = excluded.proyecto_id,
...
comment on function public.guardar_proyecto(jsonb, jsonb, jsonb) is
...
revoke all on function public.guardar_proyecto(jsonb, jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.guardar_proyecto(jsonb, jsonb, jsonb) to authenticated;
